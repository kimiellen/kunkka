...
> **历史记录：** 本计划记录 core-control protocol 的原始 core-local 实施切片。不要按本文件中的未勾选旧步骤继续执行；当前可执行计划是 `docs/superpowers/plans/2026-06-11-native-host-bridge.md`。

**目标：** 添加一个通过 Kunkka IPC 传输的最小 core control protocol，支持 Ping/Pong 和 Status/StatusResult。

**架构：** `kunkka-ipc` 继续保持 opaque transport。该计划中的 core-local control codec 是历史实施切片，已在 native-host bridge 后续计划中迁移到 `kunkka-protocol`；当前事实是 `kunkka-protocol` 拥有 core-control typed protocol/codec，`kunkka-core` 只消费它并负责 runtime dispatch。`CoreRuntime::run_once()` 按 payload schema 分发 worker registration 和 core control message。
...
以下文件清单是迁移到 `kunkka-protocol` 之前的原始实施记录，不代表当前待办。
...
- [x] 先修改 `crates/kunkka-core/tests/core_runtime_control.rs` 的导入，改为使用 `kunkka_protocol::core_control`，并运行 `cargo test -p kunkka-core --test core_runtime_control` 确认因缺少 `kunkka-protocol` 依赖而 RED。
- [x] 在 `crates/kunkka-core/Cargo.toml` 添加 `kunkka-protocol` 依赖，并从 core 移除本地 `control` 模块导出。
- [x] 更新 `crates/kunkka-core/src/error.rs`，将 protocol codec 错误来源切换为 `kunkka_protocol::ProtocolError`。
- [x] 更新 `crates/kunkka-core/src/runtime.rs`，使用 `kunkka_protocol::core_control` 的消息、codec 与 schema；构造 `CoreStatusResponse` 时将 `self.registry.len()` 显式转换为 `u64`。
- [x] 删除 `crates/kunkka-core/src/control.rs` 与 `crates/kunkka-core/tests/core_control_protocol.rs`，因为 codec 所有权已迁移到 `kunkka-protocol`。
- [x] 运行 `cargo test -p kunkka-core --test core_runtime_control`、`cargo test -p kunkka-protocol --test core_control`、`cargo fmt --all --check` 确认 GREEN。
- [x] 按 Task 2 要求暂存并提交：`refactor: use shared core control protocol`。

已由 commit `79fc475 refactor: use shared core control protocol` 完成该迁移。

## 原始历史实施步骤

以下任务 1-4 是迁移前的历史步骤，保留用于追溯，不要作为当前待办重新执行。

### 任务 1：Core control payload codec

历史步骤，迁移前在 core 内实现；不要作为当前待办重新执行。
